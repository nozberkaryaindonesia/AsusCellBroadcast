...
public class CellBroadcastAlertAudio extends Service implements TextToSpeech.OnInitListener,
    ...
    /** Extra for alert only vibration enabled (from preview alert). */
    public static final String ALERT_ONLY_VIBRATE_EXTRA =
    ...
    //+++ gary_hsu@20160223: http://pmd.asus.com:8180/display/AMAX/Cell+broadcast (20160223 new spec)
    public static final String ALERT_AUDIO_911_919_CHANNEL_EXTRA =
            "com.android.cellbroadcastreceiver.channel_911/919";
    //--- gary_hsu@20160223: http://pmd.asus.com:8180/display/AMAX/Cell+broadcast (20160223 new spec)
    ...
    private boolean mRingerModeEnableVibrate;
    private boolean mIsChannel911_919;
    ...
    // +++ gary_hsu@20160218: Spec change, http://pmd.asus.com:8180/display/AMAX/Cell+broadcast
    private static final String SKU_PROPERTIES = "ro.build.asus.sku";
    private boolean mUseMaxVolume;
    private int mAlarmVolume = 0;
    private int mTTSVolume = 0;
    private static int mAudioStreamType = AudioManager.STREAM_NOTIFICATION;
    private SKU mSKU = SKU.DEFAULT;
    public enum SKU {
        DEFAULT,
        OPEN,
        // +++ Oliver@20150605 making WW-sku having the same feature as TW-sku
        WW,
        // --- Oliver@20150605 making WW-sku having the same feature as TW-sku
        ATT,
        TW
    ...
    private void checkSku() {
        ...
        if (sku.equalsIgnoreCase("OPEN"))  mSKU = SKU.OPEN;
        else if (sku.equalsIgnoreCase("ATT")) mSKU = SKU.ATT;
        else if (sku.equalsIgnoreCase("TW")) mSKU = SKU.TW;
        // +++ Oliver@20150605 making WW-sku having the same feature as TW-sku
        else if (sku.equalsIgnoreCase("WW")) mSKU = SKU.WW;
        // --- Oliver@20150605 making WW-sku having the same feature as TW-sku
    }
    private void initAudioVibrateStatus() {
        switch (mSKU) {
            case ATT:
                mUseMaxVolume = false;
            ...
            switch (msg.what) {
                ...
                case ALERT_PAUSE_FINISHED:
                    ...
                    if (mMessageBody != null && mTtsEngineReady && mTtsLanguageSupported) {
                        ...
                        ttsHashMap.put(TextToSpeech.Engine.KEY_PARAM_UTTERANCE_ID,TTS_UTTERANCE_ID);
                        res = mTts.speak(mMessageBody, TextToSpeech.QUEUE_FLUSH, ttsHashMap);
                        // +++ andrew_tu@20140424: Customize audio/vibrate for different sku.
                        if (mUseMaxVolume) {
                            mTTSVolume = mAudioManager.getStreamVolume(AudioManager.STREAM_TTS);
                            int volume = mAudioManager.getStreamMaxVolume(AudioManager.STREAM_TTS);
                            mAudioManager.setStreamVolume(AudioManager.STREAM_TTS, volume, 0);
                        }
                        // --- andrew_tu@20140424: Customize audio/vibrate for different sku.
    ...
    public void onUtteranceCompleted(String utteranceId) {
        if (DBG) log("onUtteranceCompleted, utteranceId="+utteranceId);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        mRingerModeEnableVibrate = intent.getBooleanExtra(ALERT_AUDIO_VIBRATE_EXTRA, true);
        mIsChannel911_919 = intent.getBooleanExtra(ALERT_AUDIO_911_919_CHANNEL_EXTRA, false);
//        if (intent.getBooleanExtra(ALERT_AUDIO_ETWS_VIBRATE_EXTRA, false)) {
//            mRingerModeEnableVibrate = true;
//        }

        initAudioVibrateStatus();
        switch (mAudioManager.getRingerMode()) {
            case AudioManager.RINGER_MODE_SILENT:
                if (DBG) log("Ringer mode: silent, turn off audio and virbate");
                mEnableAudio = false;
            ...
            default:
                ...
                // +++ dion_yang@20131026:For ATT requirement
                if(intent.getBooleanExtra(ALERT_ONLY_VIBRATE_EXTRA, false)) {
                    if (DBG) log("Ringer mode: only vibrate mode");
                    mEnableAudio = false;
                } else {
                    mEnableAudio = true;
                }
                // --- dion_yang@20131026:For ATT requirement
                mEnableVibrate = mRingerModeEnableVibrate;
                break;
        }
    ...
    private void play(int duration) {
        ...
        if (mEnableAudio) {
            ...
            try {
                ...
                if(mIsChannel911_919){
                    mAudioStreamType = AudioManager.STREAM_NOTIFICATION;
                    Uri uri = RingtoneManager.getActualDefaultRingtoneUri(this, RingtoneManager.TYPE_NOTIFICATION);
                    setDataSourceFromResourceFor911_919(mMediaPlayer,uri);
                }else{
                    setDataSourceFromResource(getResources(), mMediaPlayer,
                            R.raw.attention_signal_once);
                }
                // +++ andrew_tu@20140424: Customize audio/vibrate for different sku.
                if (mUseMaxVolume) {
                    mAlarmVolume = mAudioManager.getStreamVolume(mAudioStreamType);
                    int volume = mAudioManager.getStreamMaxVolume(mAudioStreamType);
                    mAudioManager.setStreamVolume(mAudioStreamType, volume, 0);
                }
                mAudioManager.requestAudioFocus(null, mAudioStreamType,
...
//        player.setAudioStreamType(AudioManager.STREAM_NOTIFICATION);
    ...
    private void setDataSourceFromResourceFor911_919(MediaPlayer player, Uri uri)
    ...
    public void stop() {
        ...
        if (mState == STATE_ALERTING) {
            ...
            mVibrator.cancel();
            // +++ andrew_tu@20140424: Customize audio/vibrate for different sku.
            if (mUseMaxVolume) mAudioManager.setStreamVolume(mAudioStreamType, mAlarmVolume, 0);
            // --- andrew_tu@20140424: Customize audio/vibrate for different sku.
            ...
            try {
                ...
                // +++ andrew_tu@20140424: Customize audio/vibrate for different sku.
                if (mUseMaxVolume) mAudioManager.setStreamVolume(AudioManager.STREAM_TTS, mTTSVolume, 0);
                // --- andrew_tu@20140424: Customize audio/vibrate for different sku.
            }
        }
