...
public class CellBroadcastAlertFullScreen extends Activity {
    ...
    /** The cell broadcast Identifier for notification ID to display. */
    private static final int NO_NOTIFICATION_ID = -1;
    private int mNotificationId = NO_NOTIFICATION_ID;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // +++ Oliver_Ou@20150424 Fix TT-581238, Added an if-else judgement on the object message
        if(null != message ){
            ...
            // +++ joey_lee, add channels for Civil Emergency Message, show message normally (workaround, must be modified)
            } else if (message.getServiceCategory() == CellBroadcastUtils.MESSAGE_ID_CMAS_ALERT_CIVIL_EMERGENCY_MESSAGE
                    || message.getServiceCategory() == CellBroadcastUtils.MESSAGE_ID_CMAS_ALERT_CIVIL_EMERGENCY_MESSAGE_ADDITIONAL_LANGUAGES) {
                Log.d(TAG, "onCreate setting screen on timer for MESSAGE_ID_CMAS_ALERT_CIVIL_EMERGENCY_MESSAGE(911) and MESSAGE_ID_CMAS_ALERT_CIVIL_EMERGENCY_MESSAGE_ADDITIONAL_LANGUAGES(919), workaround");
                mScreenOffHandler.startScreenOnTimer();
            }
            ...
            Log.e(TAG, "mMessageList size is zero!");
        }
        // --- Oliver_Ou@20150424 Fix TT-581238
    }
    ...
    private void updateAlertText(CellBroadcastMessage message) {
        String title = CellBroadcastResources.getDialogTitleResource(this,message);
        setTitle(title);
        String messageBody = message.getMessageBody();
        ((TextView) findViewById(R.id.alertTitle)).setText(title);
        ((TextView) findViewById(R.id.message)).setText(messageBody);
        Log.v(TAG,"updateAlertText messageBody= " + messageBody);
    ...
    protected void onResume() {
        ...
        if (message != null && CellBroadcastConfigService.isEmergencyAlertMessage(message)) {
         ...
         // +++ joey_lee, add channels for Civil Emergency Message, show message normally (workaround, must be modified)
                ...
                && (message.getServiceCategory() == CellBroadcastUtils.MESSAGE_ID_CMAS_ALERT_CIVIL_EMERGENCY_MESSAGE
                || message.getServiceCategory() == CellBroadcastUtils.MESSAGE_ID_CMAS_ALERT_CIVIL_EMERGENCY_MESSAGE_ADDITIONAL_LANGUAGES)) {
            Log.d(TAG, "onResume startIconAnimation for MESSAGE_ID_CMAS_ALERT_CIVIL_EMERGENCY_MESSAGE(911) and MESSAGE_ID_CMAS_ALERT_CIVIL_EMERGENCY_MESSAGE_ADDITIONAL_LANGUAGES(919), workaround");
            mAnimationHandler.startIconAnimation();
        }
        // --- joey_lee, add channels for Civil Emergency Message, show message normally (workaround, must be modified)
    }
    ...
    public void finish() {
        super.finish();
        // +++ andrew_tu@20140324 Fix screen flicker when close CB dialog.
